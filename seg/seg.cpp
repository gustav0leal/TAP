...
void update_tree(int tree_id, int tree_l, int tree_r, int upd_id, ll upd_v)
{
    if(tree_l == tree_r)
    {
        tree[tree_id] = upd_v;
    ...
    if(upd_id <= tmid)
        update_tree(lef(tree_id),tree_l,tmid,upd_id,upd_v);
    else
        update_tree(rig(tree_id),tmid+1,tree_r,upd_id,upd_v);
